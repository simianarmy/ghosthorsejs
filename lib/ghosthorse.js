...
GhostHorse.prototype.saveProcessedTweets = function (tweets) {
    ...
    .filter(function (t) {
        return !self._tweetsToPost.find(function(tt) { return t.tid === tt.tid; });
    })
...
GhostHorse.prototype.saveTweet = function (tweet) {
    ...
    Q.delay(2000).then(function () {
        ...
        // 3. Save tweet as postable
            .then(function (data) {
                console.log('saveHorseTweet results', data);
                ...
                // saveHorseTweet promise rejection case
            }, function (reason) {
                // Means data is unprocessable - could be that it was already
                // processed or some other reason, we just know we need to
                // clear this tweet from the workflow forever.
                winston.info('saveHorseTweet REJECTED! ' + reason);
                self.tweetsConverted[tweet.tid] = tweet;
            })
        // error handler
            .catch(function (err) {
...
GhostHorse.prototype.removeFromProcessingQueue = function (tweet) {
    console.log('removeFromProcessingQueue', tweet.tid);
    // Clear from processing queue!
    var idx = this._tweetsToPost.findIndex(function(t) {
        return t.tid === tweet.tid;
    });

    if (idx >= 0) {
        console.log('Removing tweet from work queue at ', idx);
        this._tweetsToPost.splice(idx, 1);
    }
};
    ...
    if (tweet && tweet.tid) {
        ...
        this.dataStore.markHorseTweetComplete(tweet.accountId, tweet.tid, function (err, res) {
            if (!err) {
                ...
                self.tweetsConverted[tweet.tid] = tweet;
                delete self.failedTweets[tweet.tid]; // in case this was a retry
            }
            else {
