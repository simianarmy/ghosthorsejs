...
var TWITTER_USERS_TO_FOLLOW = [
    {accountId: 1, handle: 'horse_js'}
    ,{accountId: 2, handle: 'horse_horse_js'}
    //,{accountId: 3, handle: 'iamdevloper'}
    ];
...
GhostHorse.prototype.createTwitterClient = function () {
    ...
    twitter.createClient(self.configs.Twitter, function (err) {
        ...
        TWITTER_USERS_TO_FOLLOW.map(function (user) {
            twitter.trackUser(user.handle);
        });
    });
    return twitter;
};
...
GhostHorse.prototype.saveTweet = function (tweet) {
    ...
    console.log('saving to data store...', tweet);
...
GhostHorse.prototype.startTwitterReader = function () {
    ...
    self.getTweets({}, function (response) {
        if (response.length > 0) {
            console.log(response.length + ' tweets (latest): ', response[response.length-1].id);
...
GhostHorse.prototype.getHandleAccountId = function (handle) {
    var res = TWITTER_USERS_TO_FOLLOW.find(function (u) {
