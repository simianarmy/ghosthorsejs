...
TwhisprerSprinkler.prototype._processSprinklerTweet = function (tweet) {
    ...
    if (link) {
        this._expandUrl(link).then(function (url) {
            this._scanTweetUrl(url);
        }.bind(this));
    }
 ...
 */
TwhisprerSprinkler.prototype._scanTweetUrl = function (url) {
    if (!url || (url === '')) {
    ...
    if (id) {
        this._client.getStatus(id, function (err, tweet) {
            if (!err) {
