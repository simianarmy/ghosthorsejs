 ...
 */
TwhisprerSprinkler.TWEET_LINK_RE = /^@twhisprer (https:\/\/t\.co\/\S+)(?:\s*#(\w+))$/i;
...
TwhisprerSprinkler.prototype.onUpdate = function (tweets) {
    tweets.forEach(function (tweet) {
        winston.info('Twhisprer tweet found: ' + tweet.text);
...
TwhisprerSprinkler.prototype._processSprinklerTweet = function (tweet) {
    winston.info('Found new sprinkler tweet! ' + tweet.text);
    // extract tweet link
    var match = TwhisprerSprinkler.TWEET_LINK_RE.exec(tweet.text); 
    var link = match[1];

    if (link) {
        // save voice type if any
        tweet.voiceName = match[2];
...
TwhisprerSprinkler.prototype._scanTweetUrl = function (sourceTweet, url) {
    ...
    winston.info('scanning tweet link: ' + url);
...
TwhisprerSprinkler.prototype._expandUrl = function (shortUrl) {
    winston.info('expanding url ' + shortUrl);
