...
AudioUploader.prototype.upload = function (data) {
    ...
    return this._uploadToS3(data.audioFile)
        .then(function () {
            return self._uploadToSoundcloud({
                filePath: path.join(self.settings.audioPath, data.audioFile),
                author: data.handle
            });
        });
};
...
AudioUploader.prototype._uploadToSoundcloud = function (data) {
    ...
    spawn('ruby', [this.settings.soundcloudUploader, data.filePath, data.author])
