...
TwitterClient.prototype.trackUser = function (user) {
    ...
    self._max_id[user] = 0; // init max value

    self._search(user, {since_id: since});
};
    ...
    try {
        self._client.search(user, opts, function (err, data) {
            ...
            if ((typeof data !== 'undefined') && data.search_metadata.next_results) {
                ...
                self._since_id[user] = self._max_id[user] || 0;
                // start new search
                sys.log('fetching next tweets in ' + SEARCH_INTERVAL);
                setTimeout(self._search.bind(self, user, {since_id: self._since_id[user]}), SEARCH_INTERVAL);
            }
        });
    } catch (e) {
...
TwitterClient.prototype._addTweet = function (tweet) {
    ...
    this._max_id[user] = Math.max(this._max_id[user], tweet.id);
};
