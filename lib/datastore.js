...
DataStore.prototype.saveHorseTweet = function (accountId, tweet) {
    ...
    this.getHorseTweet(accountId, tweet.tid, function (err, parseObj) {
        if (err === null) {
            ...
            // If we already tweeted this one, reject this promise and let
            // caller handle cleanup
            else if (result.twitStatus === STATUS_TWEETED) {
                deferred.reject('already tweeted');
            }
            else {
