...
DataStore.prototype.saveHorseTweet = function (accountId, tweet, callback) {
    ...
    self._api.getObjects(HorseTweetClass, params, function (err, res, body, success) {
        if (!success) {
            ...
            winston.info('Saving to Parse: ' + util.inspect(tweet));

            self._api.createObject(HorseTweetClass, tweet, function (err, res, body, success) {
                if (!success) {
                    winston.info('Parse insert error: ' + err);
                    deferred.reject(err);
                } else {
                    winston.info('Saved object', body);
                    deferred.resolve(tweet, body);
                }
            });
        } else {
            //winston.info(body.count + ' Parse object(s) already exists for id: ' + tweet.tid);
